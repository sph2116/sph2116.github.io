...
let startTime 
//Set some sound variables
//let fadePoint = music1.duration - 3
console.log("music duration is" + music1.duration)
let fadeSound

function fadeAudio(){
    fadeSound = setInterval(function () {
        let fadeAmount = 0.15
        // Only fade if past the fade out point or not at zero already
        if ((music1.currentTime <= music1.duration - 3) && (music1.volume >= fadeAmount)) {
            music1.volume -= fadeAmount;
...
function tenMinTimer(){
    ...
    if(music1.currentTime < music1.duration - (endTime/1000) ){
        music1.volume = 1
        music1.play()
    } else {
        music1.currentTime = Math.random() * music1.duration
        music1.volume = 1
        music1.play()
    }
    ...
    let calcTimeLeft = setInterval(function(){
        ...
        if(timeLeft >= (endTime - 6000) && timeLeft <= (endTime - 5000) ){
            fadeAudio()
            let endtimeMinus = endTime -5000;
            console.log("fade audio at" + endtimeMinus)
        }
